...
float3 hash33(float3 p3) {
    ...
    p3 += float3(dot(p3, p3.yxz + float3(19.19)));
    return float3(-1.0) + 2.0 * fract(float3(
        p3.x + p3.y,
        p3.x + p3.z,
        p3.y + p3.z
    ) * p3.zyx);
...
float snoise3(float3 p) {
    ...
    float3 i = floor(p + float3((p.x + p.y + p.z) * K1));
    float3 d0 = p - (i - float3((i.x + i.y + i.z) * K2));
    float3 e = step(float3(0.0), d0 - d0.yzx);
    float3 i1 = e * (float3(1.0) - e.zxy);
    float3 i2 = float3(1.0) - e.zxy * (float3(1.0) - e);
    float3 d1 = d0 - (i1 - float3(K2));
    float3 d2 = d0 - (i2 - float3(K1));
    float3 d3 = d0 - float3(0.5);
    float4 h = max(float4(0.6) - float4(
        dot(d0, d0),
        dot(d1, d1),
        dot(d2, d2),
        dot(d3, d3)
...
float4 extractAlpha(float3 colorIn) {
    ...
    return float4(colorIn / float3(a + 0.00001), a);
}
